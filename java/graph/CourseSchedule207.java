    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for(int i = 0; i < numCourses; i++){
            ...
            while(!q.isEmpty()){
                ...
                for(int j = 0; j < numCourses; j++){
                    if(p[parent][j]){
                        if(p[j][parent] || visited[j] == 1) return false;
                        q.add(j);
                    }
                }
            }
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        for(int i = 0; i < numCourses; i++){
            ...
            while(!q.isEmpty()){
                ...
                List<Integer> parent = map.get(key);
                if(parent == null) continue;
                for(int p_element : parent){
                    List<Integer> child = map.get(p_element);
                    if(child != null && (child.contains(key) || child.contains(i))){
                        return false;
                    }
                    if(!q.contains(p_element)) q.add(p_element);
                }
            }
            visited[i] = true;
        }
